...
async function startSync(fixtureId) {
  const result = await query(
    ...
    [
      "odds_inplay_fixtures_bookmakers_35_raw",
...
async function finishSync(fixtureId, syncId) {
  await query(
    `select cache.finish_sync_odds_inplay($1, $2, $3)`,
    [fixtureId, BOOKMAKER_ID, syncId]
  );
}

async function upsertOddsInplayPage(syncId, fixtureId, page) {
  await query(
    `
...
export async function POST(request, context) {
  ...
  try {
    ...
    for (const page of pages) {
      await upsertOddsInplayPage(syncId, Number(fixtureId), page);
    }
    ...
    return NextResponse.json({
      ok: true,
      endpoint: `/odds/inplay/fixtures/${fixtureId}/bookmakers/${BOOKMAKER_ID}`,
