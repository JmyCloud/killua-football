...
import { fetchSportMonksPage, fetchAllSportMonksPages, FIXTURE_LEAGUES_FILTER } from "@/lib/sportmonks";
...
// Official collection endpoints (SportMonks v3 docs):
//   Team:   GET /v3/football/expected/fixtures   (include: type;fixture;participant)
//   Player: GET /v3/football/expected/lineups    (include: type;fixture;player;team)
// Both are paginated. Filter locally by fixture_id.
// ═══════════════════════════════════════════════════════════
...
async function fetchXGCollection(endpoint, queryParams, fixtureId) {
  ...
  while (page <= XG_MAX_PAGES) {
    ...
    // Once we find matching data we can stop early
...
// ═══════════════════════════════════════════════════════════
// 13. syncStandingsRound
// Endpoint: standings/rounds/{roundId}
// Include: participant;season;league;stage;round;group;rule;details.type;form
// Pagination: No
// ═══════════════════════════════════════════════════════════

export async function syncStandingsRound(roundId, refreshMode) {
  return wrapSync({
    type: "standings_round",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.standings_rounds_raw
         where round_id = $1 order by fetched_at desc limit 1`,
        [roundId]
      );
      return r.rows[0] ?? null;
    },
    refresh: async (dbQ) => {
      const syncId = await startSync("standings_rounds_raw", `round:${roundId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
        ...
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.standings_rounds_raw (round_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (round_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [roundId, JSON.stringify({ data: Array.isArray(allData) ? allData : [allData] }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:standings_round:${roundId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 14. syncStandingsCorrections
// Endpoint: standings/corrections/seasons/{seasonId}
// Include: participant;season;league;stage;group
// Pagination: No
// ═══════════════════════════════════════════════════════════

const CORRECTIONS_INCLUDE = "participant;season;league;stage;group";

export async function syncStandingsCorrections(seasonId, refreshMode) {
  return wrapSync({
    type: "standings_corrections",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.standings_corrections_raw
         where season_id = $1 order by fetched_at desc limit 1`,
        [seasonId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("standings_corrections_raw", `season:${seasonId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
          `standings/corrections/seasons/${seasonId}`,
          { include: CORRECTIONS_INCLUDE }
        );
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.standings_corrections_raw (season_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (season_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
    ...
    mode: refreshMode,
    lockKey: `sync:standings_corrections:${seasonId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 15. syncStandingsLive
// Endpoint: standings/live/leagues/{leagueId}
// Include: participant;season;league;stage;round;group;rule;details.type;form
// Pagination: No
// ═══════════════════════════════════════════════════════════

export async function syncStandingsLive(leagueId, refreshMode) {
  return wrapSync({
    type: "standings_live",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.standings_live_raw
         where league_id = $1 order by fetched_at desc limit 1`,
        [leagueId]
      );
      return r.rows[0] ?? null;
    },
    refresh: async (dbQ) => {
      const syncId = await startSync("standings_live_raw", `league:${leagueId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
        ...
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.standings_live_raw (league_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (league_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [leagueId, JSON.stringify({ data: Array.isArray(allData) ? allData : [allData] }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:standings_live:${leagueId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 16. syncCommentaries
// Endpoint: commentaries/fixtures/{fixtureId}
// Include: fixture;player;relatedPlayer
// Pagination: Yes
// ═══════════════════════════════════════════════════════════

export async function syncCommentaries(fixtureId, refreshMode) {
  return wrapSync({
    type: "fixture_commentaries",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.fixture_commentaries_raw
         where fixture_id = $1 order by fetched_at desc limit 1`,
        [fixtureId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("fixture_commentaries_raw", `fixture:${fixtureId}`, dbQ);
      try {
        // Commentaries endpoint has Pagination: No per SportMonks docs
        const resp = await fetchSportMonksPage(
        ...
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.fixture_commentaries_raw (fixture_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (fixture_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [fixtureId, JSON.stringify({ data: allData }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:commentaries:${fixtureId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 17. syncMatchFacts
// Endpoint: match-facts/{fixtureId}  (per handoff 6.6)
// Pagination: No (single fixture)
// ═══════════════════════════════════════════════════════════

export async function syncMatchFacts(fixtureId, refreshMode) {
  return wrapSync({
    type: "fixture_match_facts",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.fixture_match_facts_raw
         where fixture_id = $1 order by fetched_at desc limit 1`,
        [fixtureId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("fixture_match_facts_raw", `fixture:${fixtureId}`, dbQ);
      try {
        let allData = [];
        try {
          // Match facts has Pagination: Yes per SportMonks docs
          const pages = await fetchAllSportMonksPages(
            `match-facts/${fixtureId}`,
            { include: "type;sport;fixture", per_page: 50, page: 1 }
          );
          allData = pages.flatMap((p) => p.payload?.data ?? []);
        } catch {
          // Match facts may not be available
        }
        await dbQ(
          `insert into cache.fixture_match_facts_raw (fixture_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (fixture_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
    ...
    mode: refreshMode,
    lockKey: `sync:match_facts:${fixtureId}`,
  });
}
...
const SQUADS_INCLUDE = "player;team;season;details;position";

export async function syncSquads(seasonId, teamId, refreshMode) {
  return wrapSync({
    type: "team_squads",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.team_squads_raw
         where season_id = $1 and team_id = $2 order by fetched_at desc limit 1`,
        [seasonId, teamId]
      );
      return r.rows[0] ?? null;
    },
    refresh: async (dbQ) => {
      const syncId = await startSync("team_squads_raw", `season:${seasonId}:team:${teamId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
          `squads/seasons/${seasonId}/teams/${teamId}`,
          { include: SQUADS_INCLUDE }
        );
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.team_squads_raw (season_id, team_id, payload, fetched_at, sync_run_id)
           values ($1, $2, $3::jsonb, now(), $4)
           on conflict (season_id, team_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [seasonId, teamId, JSON.stringify({ data: Array.isArray(allData) ? allData : [allData] }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:squads:${seasonId}:${teamId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 19. syncSchedules
// Endpoint: schedules/seasons/{seasonId}/teams/{teamId}
// Pagination: No
// ═══════════════════════════════════════════════════════════

export async function syncSchedules(seasonId, teamId, refreshMode) {
  return wrapSync({
    type: "team_schedules",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.team_schedules_raw
         where season_id = $1 and team_id = $2 order by fetched_at desc limit 1`,
        [seasonId, teamId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("team_schedules_raw", `season:${seasonId}:team:${teamId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
        ...
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.team_schedules_raw (season_id, team_id, payload, fetched_at, sync_run_id)
           values ($1, $2, $3::jsonb, now(), $4)
           on conflict (season_id, team_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
    ...
    mode: refreshMode,
    lockKey: `sync:schedules:${seasonId}:${teamId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 20. syncSquadByTeam
// Endpoint: squads/teams/{teamId}
// Include: player;team;position;detailedPosition;transfer
// Pagination: No
// ═══════════════════════════════════════════════════════════

const SQUAD_TEAM_INCLUDE = "player;team;position;detailedPosition;transfer";

export async function syncSquadByTeam(teamId, refreshMode) {
  return wrapSync({
    type: "team_squads_fallback",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.team_squads_fallback_raw
         where team_id = $1 order by fetched_at desc limit 1`,
        [teamId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("team_squads_fallback_raw", `team:${teamId}`, dbQ);
      try {
        const resp = await fetchSportMonksPage(
          `squads/teams/${teamId}`,
          { include: SQUAD_TEAM_INCLUDE }
        );
        const allData = resp?.data ?? [];
        await dbQ(
          `insert into cache.team_squads_fallback_raw (team_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (team_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [teamId, JSON.stringify({ data: Array.isArray(allData) ? allData : [allData] }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:squad_team:${teamId}`,
  });
}
...
// Include: season;stage;player;participant;type
// Pagination: Yes
// ═══════════════════════════════════════════════════════════

const TOPSCORERS_INCLUDE = "season;stage;player;participant;type";

export async function syncTopscorers(seasonId, refreshMode) {
  return wrapSync({
    type: "season_topscorers",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.season_topscorers_raw
         where season_id = $1 order by fetched_at desc limit 1`,
        [seasonId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("season_topscorers_raw", `season:${seasonId}`, dbQ);
      try {
        const pages = await fetchAllSportMonksPages(
          `topscorers/seasons/${seasonId}`,
          { include: TOPSCORERS_INCLUDE, order: "asc", per_page: 50, page: 1 }
        );
        const allData = pages.flatMap((p) => p.payload?.data ?? []);
        await dbQ(
          `insert into cache.season_topscorers_raw (season_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (season_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
    ...
    mode: refreshMode,
    lockKey: `sync:topscorers:${seasonId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 22. syncTeamRankings
// Endpoint: team-rankings/teams/{teamId}
// Include: team
// Pagination: Yes
// ═══════════════════════════════════════════════════════════

export async function syncTeamRankings(teamId, refreshMode) {
  return wrapSync({
    type: "team_rankings",
    getCached: async (dbQ) => {
      const r = await dbQ(
        `select payload as data, fetched_at from cache.team_rankings_raw
         where team_id = $1 order by fetched_at desc limit 1`,
        [teamId]
      );
      return r.rows[0] ?? null;
    ...
    refresh: async (dbQ) => {
      const syncId = await startSync("team_rankings_raw", `team:${teamId}`, dbQ);
      try {
        const pages = await fetchAllSportMonksPages(
          `team-rankings/teams/${teamId}`,
          { include: "team", per_page: 50, page: 1 }
        );
        const allData = pages.flatMap((p) => p.payload?.data ?? []);
        await dbQ(
          `insert into cache.team_rankings_raw (team_id, payload, fetched_at, sync_run_id)
           values ($1, $2::jsonb, now(), $3)
           on conflict (team_id) do update set
             payload = excluded.payload, fetched_at = excluded.fetched_at,
             sync_run_id = excluded.sync_run_id, updated_at = now()`,
          [teamId, JSON.stringify({ data: allData }), syncId]
        );
        await endSync(syncId, "done", null, dbQ);
      } catch (err) {
        await endSync(syncId, "failed", err.message, dbQ);
    ...
    mode: refreshMode,
    lockKey: `sync:team_rankings:${teamId}`,
  });
}

// ═══════════════════════════════════════════════════════════
// 23. fetchFixturesLatest — discovery helper (not cached per fixture)
// Endpoint: fixtures/latest
// Pagination: No
// Returns array of recently updated fixtures for selective refresh
// ═══════════════════════════════════════════════════════════

export async function fetchFixturesLatest() {
  const resp = await fetchSportMonksPage(
    "fixtures/latest",
    { include: "state;participants;scores", filters: FIXTURE_LEAGUES_FILTER }
  );
  return resp?.data ?? [];
}
