  ...
  return wrapSync({
    ...
    refresh: async (dbQ) => {
      ...
      try {
        ...
        // Team-level xG: expected/fixtures (paginated, filter by fixture_id)
        try {
          const pages = await fetchAllSportMonksPages(
            `expected/fixtures`,
        ...
        // Player-level xG: expected/lineups (paginated, filter by fixture_id)
        try {
          const pages = await fetchAllSportMonksPages(
            `expected/lineups`,
      ...
      try {
        ...
        for (const teamId of teamIds) {
          try {
            ...
            // Team might not have expected lineups
          }
        }
        await dbQ(
